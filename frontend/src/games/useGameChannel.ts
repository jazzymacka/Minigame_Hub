...
export function useGameChannel<TState = unknown, TEvent = { type: string; [k: string]: unknown }>() {
  ...
  useEffect(() => {
    ...
    // The server may have already broadcast the initial state/events before
    // this component finished mounting and subscribing (a real race —
    // especially right as the countdown ends). Explicitly ask for a resync
    // so we're never permanently stuck without the starting state.
