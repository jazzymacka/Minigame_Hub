...
export default function MazeEscape({ lobby, playerId }: GameComponentProps) {
  const { state, sendAction } = useGameChannel<MazeState>();
  ...
  if (!state) return <p className="text-center text-ink-muted">Generating maze…</p>;

  const { maze, size, positions, finishedOrder } = state;
  const exit: [number, number] = [size - 1, size - 1];
