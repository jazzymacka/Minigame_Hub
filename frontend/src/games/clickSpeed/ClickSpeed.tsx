...
export default function ClickSpeed({ lobby, playerId }: GameComponentProps) {
  const { state, sendAction } = useGameChannel<ClickState>();
  const [timeLeft, setTimeLeft] = useState(0);
  const [myCount, setMyCount] = useState(0);

  useEffect(() => {
    if (!state) return;
    const update = () => setTimeLeft(Math.max(0, state.durationMs - (Date.now() - state.startedAt)));
    update();
    const interval = setInterval(update, 80);
    return () => clearInterval(interval);
  }, [state?.startedAt, state?.durationMs]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <div className="mx-auto max-w-xl space-y-5 text-center">
      <div className="card-surface p-4">
        <div className="mb-2 h-2 overflow-hidden rounded-full bg-elevated">
          <div className="h-full bg-accent transition-all duration-75" style={{ width: `${(timeLeft / state.durationMs) * 100}%` }} />
        </div>
