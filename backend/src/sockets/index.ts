...
import { startMatch, handleGameAction, handlePlayerLeaveMatch, endMatch, resyncPlayer } from "../state/matchRuntime.js";
import type { ChatMessage, GameId, Lobby, LobbyPlayer, PlayerSession } from "../types.js";

export function registerSocketHandlers(io: Server) {
  io.on("connection", (socket: Socket) => {
    // Defense in depth: an uncaught throw inside any socket event handler
    // would otherwise propagate as an uncaught exception and crash the
    // entire Node process — disconnecting every connected player, not just
    // the one who triggered it. Wrap every handler registered on this
    // socket so a bug in one game/feature degrades gracefully instead.
    ...
    socket.on("auth:join", (data: { sessionId?: string; username: string; avatarColour: string }) => {
      ...
      if (lobby) {
        ...
        if (lobby.status === "in-progress") {
          resyncPlayer(io, lobby, session.id);
        }
      }
    });
    ...
    socket.on("game:requestSync", () => {
      ...
      const lobby = findLobbyOfPlayer(playerId);
      if (!lobby || lobby.status !== "in-progress") return;
      resyncPlayer(io, lobby, playerId);
    });
